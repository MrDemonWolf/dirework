...
describe("buildRequestLog redaction", () => {
  it("keeps only the pathname — never the query string", () => {
    ...
    expect(log).toEqual({ id: "req-1", method: "GET", path: "/api/bot/callback/twitch", status: 302, ms: 12 });
  });
  ...
  it.each([
    ...
    ["overlay token", "https://api.example.com/trpc/overlay.getTimerState?input=%7B%22token%22%3A%22t0ken%22%7D"],
    ...
    for (const secret of ["abc123def", "csrf-nonce-xyz", "t0ken", "b0tt0ken", "?", "input=", "code=", "state=", "token="]) {
