...
The bot will have `user:read:chat` and `user:write:chat` permissions — enough to read messages and respond to commands.
...
| **Bot account** | `user:read:chat`, `user:write:chat` | Read chat messages, send bot responses |
