...
description: Create a Twitch application, sign in, and connect your bot account
---
...
**For production** (add these too if you plan to deploy — swap in your workers.dev subdomain):
```
...
3. On a fresh install you'll land on the **setup screen** — click **Claim with Twitch**
4. You'll be redirected to Twitch — authorize the app with your **streamer account**
5. After authorizing, you'll land on the dashboard as the instance owner
...
The bot will have `chat:read` and `chat:edit` permissions (plus the Helix chat scopes `user:read:chat` and `user:write:chat`) — enough to read messages and respond to commands.

<Callout>
  **One more thing before the bot talks:** the bot runs in a token-gated browser page,
  not on a server. Copy the bot page URL from **Dashboard → Bot** and add it to OBS as a
  browser source (or keep it in a pinned tab). The bot listens to chat only while that
  page is open. See [Chat commands](/docs/chat-commands) for the full command list.
</Callout>
...
| --- | --- | --- |
| **Streamer login** | `user:read:email` (Better Auth default) | Authentication, profile info |
| **Bot account** | `chat:read`, `chat:edit` | Read and send chat messages over IRC (the connection the bot page actually uses) |
| **Bot account** | `user:read:chat`, `user:write:chat` | Helix chat API equivalents — reserved for a future move to Helix sends |
...
<Callout type="warn">
  **Connected your bot before `chat:read`/`chat:edit` were added?** Existing tokens don't
  gain scopes automatically — disconnect the bot account and connect it again to pick up
  the new scopes.
</Callout>
...
3. Paste the overlay URL (it looks like `http://localhost:3001/overlay/t/YOUR_TOKEN`)
4. Set width and height to `250` (or match the timer size you set in the Theme Center)
...
| --- | --- | --- |
| `{BETTER_AUTH_URL}/api/auth/callback/twitch` | Streamer login | When you sign in with Twitch |
...
You don't need to configure access. Dirework is **single-owner**: the first Twitch
account to sign in claims the instance, and after that no one else can take it over. Your
instance is yours the moment you claim it.
...
- [ ] Set `BETTER_AUTH_URL` and `CORS_ORIGIN` to your web worker's origin — for GitHub
  Actions deploys, set these as GitHub repository **variables** (repo Settings → Secrets
  and variables → Actions → Variables); the deploy job consumes them via `${{ vars.* }}`
  in `deploy.yml` — don't edit the workflow file itself
- [ ] Add production redirect URLs to your Twitch app:
  - `https://dirework.<your-subdomain>.workers.dev/api/auth/callback/twitch`
  - `https://dirework.<your-subdomain>.workers.dev/api/bot/callback/twitch`
- [ ] Use `https://` (not `http://`) for all production URLs

The full walkthrough lives in the [Deployment guide](/docs/deployment).
