...
All data is stored in a Cloudflare D1 (SQLite) database bound to the instance operator's Cloudflare Workers deployment. Dirework does not transmit data to any external storage.

The instance operator is responsible for:

- Securing their Cloudflare account and the D1 database binding.
...
- **Tasks** — persist until cleared by the instance operator or via chat commands.
