...
6. Deploy — Coolify will build using Nixpacks, run Drizzle migrations, and start the server

### How Nixpacks Builds

The `nixpacks.toml` configures the build pipeline:

1. **Install** — Enables corepack/pnpm, installs dependencies, rebuilds native modules (esbuild, sharp)
2. **Build** — Runs Drizzle migrations, builds Next.js standalone output, copies static assets
...
<Callout type="info">
  Migrations run automatically on startup via the entrypoint script. No manual step is needed.
</Callout>
...
Migrations run automatically on container startup. For manual runs:

```bash
docker compose exec web node /app/packages/db/migrate.js
```
