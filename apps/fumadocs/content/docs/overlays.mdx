...
The timer automatically transitions through phases: starting (5s countdown) → work → break → work → ... → finished. No manual intervention needed once started — just keep the dashboard open in a tab, since it's what nudges the timer into the next phase when the countdown hits 0.
