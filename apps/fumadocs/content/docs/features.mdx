...
- **Real-time OBS overlay** — Scrolling task list with usernames in Twitch chat colors, checkboxes, and strikethrough for completed tasks
- **Auto-scroll** — Infinite scroll animation when the list overflows the display area
...
- **Task list overlay editor** — Customize header, body, task items, done state, checkboxes, bullets, fonts, scroll speed, and more
- **Live preview** — See changes instantly in the preview panel before saving
...
- **Real-time updates** — Overlays poll the server every 1-2 seconds for instant updates
...
- **Timer controls** — Start, pause, resume, skip, and stop the timer with inline duration settings
