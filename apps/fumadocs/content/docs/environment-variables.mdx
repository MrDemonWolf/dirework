...
| --- | --- | --- |
| `REDIS_URL` | _(unset)_ | Redis connection string (e.g. `redis://redis:6379`). When set, live events route through Redis pub/sub so multiple app instances stay in sync. Unset, events use the in-process bus — fine for a single container. |
...
# Redis pub/sub (only needed for multi-instance deployments)
